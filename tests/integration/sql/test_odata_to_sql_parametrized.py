...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("eac lt 10", '"eac" < ?', [10]),
        ("eac le 10", '"eac" <= ?', [10]),
        ("eac gt 1.0 and eac lt 10.0", '"eac" > ? AND "eac" < ?', [1.0, 10.0]),
        ("eac ge 1.0 and eac le 10.0", '"eac" >= ? AND "eac" <= ?', [1.0, 10.0]),
        (
        ...
        ("not (eac gt 10 and eac lt 20)", 'NOT ("eac" > ? AND "eac" < ?)', [10, 20]),
        ("eac gt 1 eq true", '("eac" > ?) = ?', [1, True]),
        ("true eq eac gt 1", '? = ("eac" > ?)', [True, 1]),
        ("eac add 10 gt 1000", '"eac" + ? > ?', [10, 1000]),
        ("eac add 10 gt eac sub 10", '"eac" + ? > "eac" - ?', [10, 10]),
        ("eac mul 10 div 10 eq eac", '"eac" * ? / ? = "eac"', [10, 10]),
        ...
        (
            ...
            'EXTRACT (YEAR FROM "supply_start_date") = EXTRACT (YEAR FROM CURRENT_TIMESTAMP) - ?',
            [1],
        ),
        (
...
def test_odata_filter_to_sql_handles_errors(
