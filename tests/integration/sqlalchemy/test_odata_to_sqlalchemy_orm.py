...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("day(published_at) eq 1", extract(BlogPost.published_at, "day") == 1),
        ("hour(published_at) eq 1", extract(BlogPost.published_at, "hour") == 1),
        ("minute(published_at) eq 1", extract(BlogPost.published_at, "minute") == 1),
        ("month(published_at) eq 1", extract(BlogPost.published_at, "month") == 1),
        ("published_at eq now()", BlogPost.published_at == functions.now()),
        ("second(published_at) eq 1", extract(BlogPost.published_at, "second") == 1),
        (
            "time(published_at) eq 14:00:00",
            cast(BlogPost.published_at, Time) == dt.time(14, 0, 0),
        ),
        ("year(published_at) eq 2019", extract(BlogPost.published_at, "year") == 2019),
